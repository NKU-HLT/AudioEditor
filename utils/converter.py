...
def normalize_spectrogram(
    ...
    assert spectrogram.max() <= max_value 
    assert spectrogram.min() >= min_value
...
def get_mel_spectrogram_from_audio(audio, device="cuda"):
    ...
    if len(audio.shape) == 3:
        audio_reduced = audio[:, :, 0]
    else:
        audio_reduced = audio
    # print(f"Processed audio_reduced shape: {audio_reduced.shape}")
    target_length = 160000
    current_length = audio_reduced.size(1)
    if current_length < target_length:
        padding = target_length - current_length
        audio_reduced = F.pad(audio_reduced, (0, padding), mode='constant', value=0)
    elif current_length > target_length:
        audio_reduced = audio_reduced[:, :target_length]
    # print(f"Processed audio_reduced shape: {audio_reduced.shape}")


    waveform = audio_reduced.to(device)
